...
across agents rather than what its average is.
...
pre-LLM history. \RESULT{Report the exact share of the \RepoFrame{} curated repositories
created before \LlmCutoff{}, measured over all of them by
analysis/01\_corpus/verify\_repo\_ages.py; a preliminary 25-repository sample gave
roughly 71\%.} Repositories too young to have a pre-cutoff history are excluded from the
matched analysis, and this exclusion is not random --- newer projects plausibly differ
in their propensity to adopt agents. We therefore report a generalisation check on the
excluded repositories using positives alone.
