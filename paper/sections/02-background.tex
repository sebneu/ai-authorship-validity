...
\citednum{40,000} articles to estimate the growth of LLM use in journalism, the same
instrument stack as the software prevalence literature. They also supply the design
precedent for this paper: they score roughly \citednum{17,000} Washington Post articles
published between \citednum{2012} and \citednum{2017}, where any detection is a false
positive by construction, and use the result to bound the error of their estimate. Our
study transfers that design to software artifacts, extends it from prose to code, and
adds the step they do not take, which is propagating the measured error into the
prevalence estimate itself.
...
research problem \citep{golzadeh2021bodegha}. The confounder is measurable: in
...
Provenance has a standard vocabulary. PROV-O \citep{lebo2013provo} models entities,
activities and agents together with the relations that connect them, and it is the base
this paper builds on. Software supply-chain frameworks apply the same idea to builds,
recording which toolchain, inputs and environment produced a released artifact so that a
consumer can verify the chain.

Both answer a different question from the one at issue here. Supply-chain provenance
establishes what produced an artifact, under the assumption that the producer is known
and attests to its own output. Authorship attribution asks who or what wrote the
content, in a setting where the answer is often inferred rather than attested and where
the inference has a measurable error rate. Neither vocabulary has a place to record that
an attribution came from a detector, at which threshold, or how often that detector is
wrong on artifacts of that kind. Section~\ref{sec:provenance} adds one.
