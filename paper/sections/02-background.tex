...
Two properties of this family matter here. First, all of it is calibrated on text of
substantial length; the statistical quantities being estimated are sample means over
tokens, and their variance grows as texts shorten. Second, the null hypothesis is
``written by a human'', not ``not written by an LLM''. Templated, generated or
boilerplate text produced by non-LLM automation occupies the same low-perplexity region
of the distribution as LLM output, and nothing in the method distinguishes them.
...
\emph{which} of five agents authored a pull request. The task differs from ours in a way
that makes the two complementary rather than competing: it is a multi-class
discrimination among agents, evaluated on contributions already known to be
agent-authored, and it therefore says nothing about the agent-versus-human decision or
about false positives on human text. We include it in our evaluation as a supervised,
in-domain reference point --- an estimate of the ceiling achievable when a classifier is
trained on exactly the distribution it is tested on, against which the off-the-shelf
zero-shot detectors used in the prevalence literature can be read.
...
\end{equation}
The estimator makes the dependence explicit and occasionally embarrassing: when
$s + c - 1$ is small --- that is, when the test barely outperforms chance --- the
correction amplifies uncertainty without bound, and the corrected estimate is
uninformative. Applying it to the published prevalence figures requires exactly the
quantities that have not been measured for this domain, which is what
Section~\ref{sec:validity} supplies.

\subsection{Bots and pre-LLM automation}
\label{sec:background:bots}

Machine-generated text in open-source repositories long predates large language models.
Dependency managers, release automation, continuous integration reporters and templated
issue forms have been producing text at scale for over a decade, and identifying them
is itself an established research problem \RESULT{BoDeGHa citation, verify}. The
confounder is not hypothetical: in AIDev, \AidevCommentBotShare{} of the comments on
agent-authored pull requests are themselves bot-authored. Any prevalence estimate that
does not separate bot text from LLM text is measuring their sum, and any detector
evaluation that does not report performance on bot text separately cannot say which of
