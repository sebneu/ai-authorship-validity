...
The preceding sections produced a specific kind of number. The false positive rate of
Binoculars is not one figure but one per genre and threshold, running from
\PublishedBinoMinFpr{} to \PublishedBinoMaxFpr{} per cent at the published operating
point alone (Table~\ref{tab:published}), and Youden's $J$ varies the same way
(Table~\ref{tab:youden}). A detector score is therefore only interpretable alongside the
stratum it came from.

Nothing in current practice carries that stratum. A prevalence figure, a dashboard flag
or a moderation decision records that an artifact was classified as AI-written and
discards the grounds, so the error rate measured here stops travelling the moment the
measurement ends. This section describes a small vocabulary that keeps the two together,
and shows the questions that become answerable once it does. It is the one part of this
paper that is a design rather than a measurement, and it exists because the measurements
above are useless to a consumer who cannot see which stratum a claim belongs to.
...
Tables~\ref{tab:published} and~\ref{tab:youden} are loaded into a graph over a sample of the
...
The example of Section~\ref{sec:validity} is exactly such a case. It carries no declared
evidence, because no agent wrote it and nobody claimed one did, and \ExFlagged{} pieces
of detector evidence, each annotated with the false positive rate of the stratum it came
from. The first query returns it. A consumer of that graph can see that the only grounds
for calling the message AI-written are three scores drawn from strata where the
instruments are known to be wrong at the rates in Table~\ref{tab:published}, without
re-running anything and without access to the text.
...
pull request bodies, which matches Table~\ref{tab:recall}.
